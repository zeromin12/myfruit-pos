...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama === '' || $hargaJual <= 0 || $kode === '') {
        $error = 'Kode, nama produk, dan harga jual wajib diisi dengan benar.';
    } else {
        ...
        if (!empty($_FILES['gambar']['name'])) {
            ...
            if (in_array($ext, ['jpg','jpeg','png','webp'])) {
                ...
                move_uploaded_file($_FILES['gambar']['tmp_name'], __DIR__ . '/../assets/img/' . $namaFile);
            }
        }

        try {
            $stmt = $pdo->prepare("UPDATE produk SET kode_produk=?, nama_produk=?, kategori_id=?, satuan=?, harga_beli=?, harga_jual=?, stok=?, stok_minimum=?, gambar=?, status=? WHERE id=?");
            $stmt->execute([$kode, $nama, $kategoriId, $satuan, $hargaBeli, $hargaJual, $stok, $stokMin, $namaFile, $status, $id]);
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="kategori_id" class="form-select">
                        ...
                        <?php foreach ($kategoriList as $k): ?>
                            <option value="<?= $k['id'] ?>" <?= $k['id'] == $produk['kategori_id'] ? 'selected' : '' ?>><?= htmlspecialchars($k['nama_kategori']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    ...
                    <select name="satuan" class="form-select">
                        <?php foreach (['kg','pcs','ikat','pack','box'] as $s): ?>
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="harga_beli" class="form-control" value="<?= $produk['harga_beli'] ?>">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" step="0.01" name="stok" class="form-control" value="<?= $produk['stok'] ?>">
                </div>
                <div class="col-md-3">
                    <label class="form-label small fw-semibold">Stok Minimum</label>
                    <input type="number" step="0.01" name="stok_minimum" class="form-control" value="<?= $produk['stok_minimum'] ?>">
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <?php if ($produk['gambar']): ?>
                        <img src="../assets/img/<?= htmlspecialchars($produk['gambar']) ?>" class="mt-2 rounded" style="height:60px;">
                    <?php endif; ?>
                </div>
            </div>

            <div class="mt-4">
                <button type="submit" class="btn btn-mf"><i class="bi bi-save me-1"></i>Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
